  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {data &&
        ...
        ((data as Record<string, unknown>).source as string).startsWith("v1/") && (
          <div className="rounded-3xl border border-sky-200 bg-sky-50 p-4 text-xs text-sky-700">
            Self-hosted usage metrics are pulled from `/v1/team/credit-usage` and `/v1/team/token-usage`.
          </div>
        )}
      ...
      {data ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <dl className="grid gap-6 md:grid-cols-2">
            {Object.entries(data)
              ...
              .map(([key, value]) => (
                <div key={key} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 shadow-inner">
        ...
        !isLoading && <p className="text-sm text-slate-500">No usage data returned.</p>
      )}
    </div>
  );
}
