...
export default function WhatsNewPage() {
  ...
  if (unsupported) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 text-sm text-slate-300">
        No changelog endpoint detected. Keep an eye on your deployment notes for
        updates.
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-rose-500/60 bg-rose-500/10 p-6 text-sm text-rose-100">
  ...
  if (!entries.length) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 text-sm text-slate-300">
        No changelog entries were returned. Add a changelog to your Firecrawl
        instance to populate this view.
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="space-y-6">
        {entries.map((entry, index) => (
          ...
          >
            <header className="flex flex-wrap items-center gap-3 text-sm text-slate-300">
              {entry.version && (
